...
def maybe_generate_history(question_id: str, student_ability: str, prob_attempted=0.3) -> List[Dict]:
...
def generate_all_student_histories():
    ...
    for i, student in enumerate(all_students):
        ...
        # Only generate history for the specific paper this student record represents
...
if all_records:
    ...
    # =================================================================
    print(f"\n🔄 STARTING AUTOMATIC NORMALIZATION...")
    print("=" * 60)
    ...
    try:
        ...
        # Run normalization on the generated CSV file
        print(f"📊 Normalizing: {csv_filename}")
        normalized_tables = create_normalized_schema(csv_filename)

        if normalized_tables:
            print(f"\n✅ Normalization completed successfully!")
            print(f"📁 Generated {len(normalized_tables)} normalized tables")

            # Calculate file sizes for cleanup decision
            csv_size = os.path.getsize(csv_filename) / (1024*1024)  # MB
            parquet_size = os.path.getsize(parquet_filename) / (1024*1024)  # MB
            total_flat_size = csv_size + parquet_size

            print(f"\n💾 Storage Analysis:")
            print(f"   • Flat CSV file: {csv_size:.1f} MB")
            print(f"   • Flat Parquet file: {parquet_size:.1f} MB")
            print(f"   • Total flat files: {total_flat_size:.1f} MB")
            print(f"   • Normalized tables: ~{total_flat_size * 0.3:.1f} MB (estimated)")

            # Optional: Clean up large flat files to save space
            cleanup_large_files = input(f"\n🗑️  Delete large flat files to save {total_flat_size:.1f} MB of space? (y/N): ").strip().lower()

            if cleanup_large_files == 'y':
                try:
                    print(f"\n🗑️  Cleaning up large flat files...")
                    print(f"   • Removing {csv_filename}")
                    print(f"   • Removing {parquet_filename}")
                    print(f"   • Keeping {students_filename} (reference data)")

                    os.remove(csv_filename)
                    os.remove(parquet_filename)
                    # Keep students_filename as it's smaller and useful for reference

                    print(f"✅ Cleaned up {total_flat_size:.1f} MB of disk space")
                    print(f"💡 Use normalized tables for all analysis and production work")

                except Exception as e:
                    print(f"⚠️  Warning: Could not delete flat files: {e}")
            else:
                print(f"📁 Keeping flat files for backup/analysis")
                print(f"💡 You can manually delete them later to save space")

        else:
            print(f"❌ Normalization failed - keeping flat files for manual processing")

    except ImportError:
        print(f"⚠️  create_normalized_schema.py not found - skipping normalization")
        print(f"💡 Run 'python create_normalized_schema.py {csv_filename}' manually to normalize")
    except Exception as e:
        print(f"❌ Error during normalization: {e}")
        print(f"💡 Flat files preserved - you can normalize manually")
    ...
    print(f"\n🚀 System Ready For:")
    print(f"   ✅ PostgreSQL migration (use normalized tables)")
    print(f"   ✅ Learning analytics and insights")
    print(f"   ✅ Individual student progress tracking")
    print(f"   ✅ Real student account integration")
    print(f"   ✅ Production learning management system")

    print(f"\n📁 Next Steps:")
    print(f"   1. Use enriched_vector.py for learning recommendations")
    print(f"   2. Run postgresql_migration.sql to deploy database")
    print(f"   3. Clear simulation data when ready for real students")
    print(f"   4. Add P2, P3, P4, P5 question data as available")
