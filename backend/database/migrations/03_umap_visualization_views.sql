...
-- Table for student question status with 2D UMAP coordinates (converted from materialized view)
CREATE TABLE student_question_status AS
...
ALTER TABLE student_question_status ADD COLUMN id SERIAL PRIMARY KEY;
ALTER TABLE student_question_status ADD CONSTRAINT unique_student_question
    UNIQUE (student_id, internal_question_id);

-- Performance indexes for the table
...
-- Function to refresh student status table (full rebuild)
CREATE OR REPLACE FUNCTION refresh_student_status_view()
RETURNS void AS $$
BEGIN
    -- For a table, we truncate and rebuild all data
    TRUNCATE student_question_status;

    INSERT INTO student_question_status (
        student_id, question_id, internal_question_id, x_coord, y_coord,
        status, attempt_count, correct_count, skipped_count, total_time_spent,
        avg_confidence, latest_attempt_time, latest_confidence, latest_time_spent,
        color_code, question_number, text_length, paper_code, paper_name,
        text_preview, question_updated_at, view_updated_at
    )
    WITH latest_attempts AS (
        SELECT
            sqh.enrollment_id,
...
BEGIN
    ...
    WITH latest_attempts AS (
        ...
        WHERE spe.student_id = target_student_id::INTEGER
    ),
    aggregated_attempts AS (
        SELECT
            sqh.enrollment_id,
    ...
    FROM questions q
    JOIN papers p ON q.paper_id = p.paper_id
    JOIN student_paper_enrollments spe ON spe.student_id = target_student_id::INTEGER
        AND spe.paper_id = q.paper_id  -- Only get enrollment for this specific paper
    LEFT JOIN aggregated_attempts aa ON aa.enrollment_id = spe.enrollment_id
        AND aa.internal_question_id = q.internal_question_id
    LEFT JOIN latest_attempts la ON la.enrollment_id = spe.enrollment_id
        AND la.internal_question_id = q.internal_question_id
    ...
    WHERE q.umap_2d_embedding IS NOT NULL
        AND spe.is_active = TRUE
    ON CONFLICT (student_id, internal_question_id)
    DO UPDATE SET
        ...
        latest_time_spent = EXCLUDED.latest_time_spent,
...
-- Grant necessary permissions for table
GRANT SELECT, INSERT, UPDATE, DELETE ON student_question_status TO PUBLIC;
GRANT USAGE, SELECT ON SEQUENCE student_question_status_id_seq TO PUBLIC;
